...
public class CheckPhoneServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Lấy giá trị số điện thoại từ yêu cầu POST
        String phone = request.getParameter("phone");

        // Kiểm tra xem số điện thoại đã tồn tại trong cơ sở dữ liệu hay chưa
        boolean isDuplicate = validation.checkPhoneNumber(phone);

        // Trả về kết quả dưới dạng chuỗi "duplicate" hoặc "unique" bằng AJAX
        if (!isDuplicate) {
            out.print("duplicate");
        } else {
            out.print("unique");
